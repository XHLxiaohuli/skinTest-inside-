...
export default {
    props: {
        ...
        weeks: {
            type: Array,
            default:function(){
                return (window.navigator.language || window.navigator.browserLanguage).toLowerCase() == "zh-cn"?['日', '一', '二', '三', '四', '五', '六']:['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
            }
        },
        // 自定义月份
        months:{
            type: Array,
            default:function(){
                return (window.navigator.language || window.navigator.browserLanguage).toLowerCase() == "zh-cn"?['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']:['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
            }
        },
        // 自定义事件
        events:  {
    ...
    methods: {
        ...
        changeYear(){
            ...
            this.yearsShow=true
            this.years=[];
            for(let i=~~this.year-80;i<~~this.year+1;i++){
                this.years.push(i)
            }
        },
        selectYear(value){
